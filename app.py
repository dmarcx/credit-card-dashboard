...
def main() -> None:
    ...
    with st.sidebar:
        ...
        if df_all is not None:
            ...
            st.caption("🔒 קבצי PDF לא מועלים לשרת")

        st.divider()
        with st.expander("❓ איך משתמשים?"):
            st.markdown("""
**העלאת קבצים**
- לחץ על "Browse files" והעלה קובץ PDF אחד או יותר של פירוט כרטיס אשראי.
- ניתן להעלות **כמה כרטיסים** יחד (כאל + ישראכרט) וכן **כמה חודשים** בבת אחת — הנתונים יאוחדו אוטומטית.

**סינון חודשים וקטגוריות**
- לאחר הטעינה יופיעו אפשרויות סינון בתפריט הצד.
- ניתן לבחור חודש אחד, מספר חודשים, או הכל.
- סינון קטגוריות מאפשר להתמקד בתחום הוצאה ספציפי.

**גרפים אינטראקטיביים**
- לחץ על **כפתורי הקטגוריות** מתחת לגרפים כדי לסנן את כל הדשבורד לאותה קטגוריה.
- לחץ שוב כדי לבטל את הסינון.
- עמוד על הגרפים לפרטים נוספים.

**תובנות וחריגות**
- 🔁 **הוצאות קבועות** — ספקים שמופיעים בחודשים רבים.
- ⚡ **חד פעמיות** — רכישות גדולות שבוצעו פעם אחת.
- 🚨 **חריגות** — עסקאות חריגות ביחס לממוצע הקטגוריה.

**חיפוש עסקאות**
- בתחתית הדף ניתן לחפש עסקאות לפי שם בית העסק.
